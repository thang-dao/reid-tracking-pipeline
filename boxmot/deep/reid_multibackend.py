...
class ReIDDetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='osnet_x0_25_msmt17.pt', device=torch.device('cpu'), fp16=False):
        ...
        self.pt, self.jit, self.onnx, self.xml, self.engine, self.tflite, self.tar = self.model_type(w)  # get backend
        self.fp16 = fp16
    ...
    def model_type(p='path/to/model.pt'):
        ...
        from boxmot.deep.reid_export import export_formats
        sf = list(export_formats().Suffix)
        sf.append('.tar')  # export suffixes
...
if __name__ == '__main__':
    ...
    model_weights = Path('/storage/hieunmt/multi_camera_tracking/convert_pth_tar_to_pt/pth_tar/osnet_ain_ms_d_c.pth.tar')
    device = 'cuda:1'

    model = ReIDDetectMultiBackend(weights=model_weights, device=device, fp16=False)
